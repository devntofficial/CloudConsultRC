using CloudConsult.Notification.Events.Member;
...
namespace CloudConsult.Notification.Consumers.Member
{
    public class ProfileCreatedConsumer : IHostedService
    {
            ...
            IServiceProvider serviceProvider, IConfiguration config)
        {
            ...
            topicName = config["KafkaConfiguration:ConsumerTopics:MemberProfileCreatedConsumer"];
            this.emailTemplate = "Templates/Member/ProfileCreatedEmail.cshtml";
        }
        ...
        public async Task ProcessMessage(RawKafkaRecord record)
        {
            ...
            {
                logger.LogCritical("Error occured when consuming member profile created event");
