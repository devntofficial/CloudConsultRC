...
{
    public class RejectConsultationHandler : ICommandHandler<RejectConsultation>
    {
        private readonly IApiResponseBuilder builder;
        private readonly IConsultationService consultationService;
        private readonly IValidator<RejectConsultation> validator;

        public RejectConsultationHandler(IApiResponseBuilder builder, IConsultationService consultationService,
            IValidator<RejectConsultation> validator)
        {
        ...
        public async Task<IApiResponse> Handle(RejectConsultation request, CancellationToken cancellationToken)
        {
            ...
            var response = await consultationService.Reject(request.ConsultationId, cancellationToken);
